...
window = pygame.display.set_mode((490,490), pygame.RESIZABLE)
...
def render(prom, sel):
    ...
    if sel is not None:
        pygame.draw.rect(window, (255, 255, 0), [sel[0] * 60 + 5, 60 * (7 - sel[1]) + 5, 60, 60])
    ...
    for col in range(8):
        ...
        for row in range(8):
            ...
            if not number(game_state[key][val_dex]):
                pygame.draw.circle(window, (255, 0, 0), (35 + 60 * col, 35 + 60 * (7 - row)), 20)
                #window.blit(pygame.image.load(f"Chess PNG's/{name(game_state[key][val_dex])}.png"), (5 + 60 * col, 5 + 60 * (7 - row)))

    if prom is not None:
        key, val_dex = position(prom[0], prom[1], game_state)
        
        if colour(game_state[key][val_dex]) == "w":
            pygame.draw.rect(window, (184, 184, 184), [4 + 60 * prom[0], 5, 62, 242])
            window.blit(pygame.image.load(f"Chess PNG's/White queen.png"), (5 + 60 * prom[0], 5))
            window.blit(pygame.image.load(f"Chess PNG's/White rook.png"), (5 + 60 * prom[0], 5 + 60))
            window.blit(pygame.image.load(f"Chess PNG's/White bishop.png"), (5 + 60 * prom[0], 5 + 60 * 2))
            window.blit(pygame.image.load(f"Chess PNG's/White knight.png"), (5 + 60 * prom[0], 5 + 60 * 3))
        
        else:
            pygame.draw.rect(window, (184, 184, 184), [4 + 60 * prom[0], 3 + 60 * 4, 62, 242])
            window.blit(pygame.image.load(f"Chess PNG's/Black queen.png"), (5 + 60 * prom[0], 5 + 60 * 7))
            window.blit(pygame.image.load(f"Chess PNG's/Black rook.png"), (5 + 60 * prom[0], 5 + 60 * 6))
            window.blit(pygame.image.load(f"Chess PNG's/Black bishop.png"), (5 + 60 * prom[0], 5 + 60 * 5))
            window.blit(pygame.image.load(f"Chess PNG's/Black knight.png"), (5 + 60 * prom[0], 5 + 60 * 4))
